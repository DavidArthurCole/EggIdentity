...
try { registry = AgentRegistry.LoadFromDir(configDir); }
catch (Exception e) { Console.Error.WriteLine($"eggidentity-agent: load config dir: {e.Message}"); return 1; }

TimeSpan interval;
try { interval = AgentConfig.ParseDuration(intervalStr); }
catch (Exception e) { Console.Error.WriteLine($"eggidentity-agent: AGENT_WATCH_INTERVAL: {e.Message}"); return 1; }
...
app.MapPost("/deploy/{appName}", async (string appName, HttpRequest req) => {
    ...
    var ok = secret != "" && CryptographicOperations.FixedTimeEquals(
